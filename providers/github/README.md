...
| PR screenshots | GitHub user-attachments via `gh image` (extension `drogers0/gh-image`), never a branch — `screenshots <pr> <files>` uploads and rewrites the body's `## Screenshots` section (UI changes only) |
