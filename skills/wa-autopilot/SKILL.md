...
- **Delivery** — commit (task file `phase: review` included), **push** branch, then open **draft PR** so user reviews on GitHub: `gh pr create --draft --assignee @me --base <fork point> --head <branch> --title "<ticket title>" --body` = summary + `Closes #<n>` + acceptance criteria checklist + line `Draft — autopilot delivery, verifier not run yet. Test, then /wa-feedback · /wa-validate · /wa-close.` Base = branch ticket forked from (sprint branch, else `close.target`). **Check base hooks first** — `git show origin/<base>:.github/workflows/whackagent-board.yml | head -1` → `template version: <v>`. `v < 3` (or file missing) → **no draft**: older hooks treat any opened PR as ready, jump ticket to `ready-to-merge` and drop coding claim. Push only, say in report card `no draft: <base> carries hooks v<v> — /wa-setup backlog to upgrade`. Stacked base (another ticket's branch forked before upgrade) hits this most. Print URL in report card header. Draft moves nothing on board (hook ignores drafts), coding claim stays held: ticket still in whackagent loop (test → feedback → validate). Draft already open for branch → push only. Never push a ticket with open **non-draft** PR (`gh pr list --head <branch> --json isDraft`): `synchronize` would drop coding claim. Ready-for-review = `/wa-close`, after user validates.
