...
- **Delivery** — commit (task file `phase: review` included), then **push** branch: work visible and safe on remote. Safe only because autopilot takes `grilled` tickets alone — no open PR, so push moves nothing on board. Never push a ticket with open PR (`gh pr list --head <branch>`): `synchronize` would drop coding claim. No PR — PR = `/wa-close`, after user validates. Coding claim stays held: ticket still in whackagent loop (test → feedback → validate).
