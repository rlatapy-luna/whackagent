...
4. **Push + PR** — `git push --force-with-lease` (rebased), then `gh pr create --base <base> --head <branch> --title "<ticket title>" --body` = summary + `Closes #<n>` + acceptance criteria checklist. **Existing draft PR** for branch (normal case: every round pushed to it) → push, refresh body (`gh pr edit`), then `gh pr ready <pr>` — draft = your test, ready = your merge. **Existing open non-draft PR** (fix round) → push only, no new PR. Plan block names PR base and says it pushes; always confirmed, every time.
